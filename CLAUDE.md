...
Request flow: `app/main.py` (app, `/health`, and the `log_request` middleware wrapping everything below) → `app/routers/chat.py` (`POST /chat`, Pydantic request/response models; router dependencies run `app/auth.py` then `app/rate_limit.py`, then the handler body calls `app/guardrails.py`, then `app/routing.py`, then `check_reply` from `app/guardrails.py` on the reply) → `app/services/llm_client.py` (the Gemini call).
...
- **Output guardrails withhold, and say nothing about why.** `check_reply` in `app/guardrails.py` runs the input PII detectors (following `GUARDRAIL_CHECKS`, minus `injection`) plus a secret check (every `GATEWAY_API_KEYS` value and `GOOGLE_API_KEY`) on the reply, and raises `HTTPException(502)` with the constant `Reply withheld by the gateway.`, distinct from the upstream-failure string. It sits in the handler *after* `generate_with_fallback` returns and outside the `try`, so a block can neither trigger the tier retry nor be remapped by an `except`. Secrets match as whole values only (a prefix match would be an oracle) and empty values are skipped (`""` is in every string). The detail is never built from the reply, and the log line carries the `output_guardrail` reason, category names and `key_id` only. `tests/test_output_guardrails.py` pins all of it, and `tests/test_llm_client.py` has tripwires that fail if a system instruction, extra `contents`, or a configured secret is ever sent to the model.
- **Detectors must stay linear.** They run synchronously on the event loop, and replies are not capped by `MAX_MESSAGE_LENGTH`. The email pattern is bounded on both sides and card windows are sliced to their length, never `digits[start:]`; the quadratic versions took ~24s on 64k characters. `_CARD_FORMATS` has no `^` anchors because it is applied with `pattern.match(digits, start)`.
...
- **One summary line per `/chat` request, from middleware.** `log_request` in `main.py` writes one JSON line to stdout on logger `app.request_log`, whatever the outcome: 401, 429, 400, 422, 502, and 500 via its `finally`. The `logger.warning()` calls in the other modules stay as the failure diagnostics. Middleware can't take `Depends`, so its data arrives through `request.state`, which is a view onto the scope dict shared with the handler's `Request`. Only two places write it: `require_api_key` sets `key_id`, on success only, and `chat()` sets `model` right after routing returns and before `check_reply`, so a withheld reply still names its tier. Lifespan state must never set either key. With no `key_id`, the line says `"unauthenticated"`: no identity was established. That includes a malformed-JSON 422 sent with a valid key, because FastAPI parses the body before dependencies run. The line is a closed allowlist of seven fields: `ts`, `key_id`, `method` (clamped to the standard methods, anything else is `OTHER`), `path` (the literal `/chat`), `status`, `latency_ms` and `model`. None of them is read from headers, the query string, either body or exception text. `tests/test_request_log.py` pins both the keys and the values. The handler looks up `sys.stdout` on every write; a plain `StreamHandler(sys.stdout)` keeps pytest's capture stream, and `capsys` then sees nothing.
...
Auth: yes. Rate limiting: yes, per client key, in memory. `/chat` requires a key from `GATEWAY_API_KEYS` (see `app/auth.py`) and is limited per key (see `app/rate_limit.py`); `/health` stays public and unlimited. `ChatRequest.message` is capped at `MAX_MESSAGE_LENGTH` (16,000 chars), because the limiter counts requests while Gemini bills tokens. Input guardrails reject obvious PII and casual prompt-injection attempts with a `400`, and output guardrails withhold a reply carrying obvious PII or a configured secret with a `502` (see `app/guardrails.py`). The service is meant to run on localhost only.

Every `/chat` request writes one JSON summary line to stdout (see the request-log convention above), and nothing else: there is no log aggregator, which is out of scope for a localhost gateway.

Seven known gaps, documented on purpose rather than fixed:
...
- **The output check is a tripwire, not a boundary.** No secret is ever sent to the model, so a secret can only reach a reply if the caller pasted it into the prompt, and by then it has already gone to Google; the check cannot undo that. PII in a reply is fabricated or example data, and ordinary examples (`user@example.com`, the `4242 4242 4242 4242` test card, `555-123-4567`) are withheld too, accepted and pinned rather than allowlisted. A caller who wants content out asks for it spaced, spelled out or base64'd, which walks past it. A blocked reply has already cost one or two upstream calls and a rate-limit slot, and generic HTTP clients may retry the `502`. It earns more once a system prompt, tools or retrieval exist, and exact matching will still miss a paraphrased prompt.
...
- **The request log is a summary, not an audit trail.**
  - `latency_ms` stops when the response starts, not when the body finishes sending.
  - A client disconnect is logged as `500`.
  - A root log handler (`--log-config`, `basicConfig`) prints every line twice.
  - Every 401 writes two lines, the auth warning plus the summary, and nothing limits unauthenticated volume.
  - `key_id` is unsalted SHA-256, so leaked logs let someone test a guessed low-entropy key offline. The upgrade is an HMAC with a server secret.
  - uvicorn's own access log still prints the query string, so the no-leak guarantee covers `app.request_log` only.
